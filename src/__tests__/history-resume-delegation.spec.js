...
describe("History resume delegation - parent metadata transitions", () => {
    ...
    it("reopenParentFromDelegation continues when overwrite operations fail and still resumes/emits (RPD-06)", async () => {
        ...
        const provider = {
            contextProxy: { globalStorageUri: { fsPath: "/tmp" } },
            getTaskWithId: vi.fn().mockImplementation(async (id) => {
                if (id === "parent-rpd06") {
                    return {
                        historyItem: {
                            id: "parent-rpd06",
        ...
        expect(parentInstance.resumeAfterDelegation).toHaveBeenCalledTimes(1);
        expect(emitSpy).toHaveBeenCalledWith(RooCodeEventName.TaskDelegationCompleted, "parent-rpd06", "child-rpd06", "Subtask finished despite overwrite failures");
    ...
    it("reopenParentFromDelegation logs child status persistence failure and continues reopen flow (RPD-04)", async () => {
        ...
        const updateTaskHistory = vi.fn().mockImplementation(async (historyItem) => {
        ...
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("[reopenParentFromDelegation] Failed to persist child completed status for child-rpd04:"));
        expect(updateTaskHistory).toHaveBeenCalledWith(expect.objectContaining({
            id: "parent-rpd04",
            status: "active",
