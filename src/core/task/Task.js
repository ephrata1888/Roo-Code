...
export class Task extends EventEmitter {
    ...
    async addToApiConversationHistory(message, reasoning) {
        ...
        if (message.role === "assistant") {
            ...
            const apiProtocol = getApiProtocol(apiProvider && !isRetiredProvider(apiProvider) ? apiProvider : undefined, modelId);
    ...
    async flushPendingToolResultsToHistory() {
        ...
        if (!this.assistantMessageSavedToHistory) {
            await pWaitFor(() => this.assistantMessageSavedToHistory || this.abort, {
                ...
                console.warn(`[Task#${this.taskId}] flushPendingToolResultsToHistory: timed out waiting for assistant message to be saved`);
            });
        }
        ...
        if (saved) {
            ...
            console.warn(`[Task#${this.taskId}] flushPendingToolResultsToHistory: save failed, retaining pending tool results in memory`);
        }
        return saved;
    }
    ...
    async retrySaveApiConversationHistory() {
        ...
        for (let attempt = 0; attempt < delays.length; attempt++) {
            ...
            console.warn(`[Task#${this.taskId}] retrySaveApiConversationHistory: retry attempt ${attempt + 1}/${delays.length}`);
    ...
    async getFilesReadByRooSafely(context) {
        try {
    ...
    async condenseContext() {
        ...
        const { messages, summary, cost, newContextTokens = 0, error, errorDetails, condenseId, } = await summarizeConversation({
    ...
    async recursivelyMakeClineRequests(userContent, includeFileDetails = false) {
        ...
        while (stack.length > 0) {
            ...
            const { showRooIgnoredFiles = false, includeDiagnosticMessages = true, maxDiagnosticMessages = 50, } = (await this.providerRef.deref()?.getState()) ?? {};
            ...
            try {
                ...
                if (hasTextContent || hasToolUses) {
                    ...
                    if (newTaskIndex !== -1 && newTaskIndex < assistantContent.length - 1) {
                        ...
                        const executionNewTaskIndex = this.assistantMessageContent.findIndex((block) => block.type === "tool_use" && block.name === "new_task");
                        ...
                        for (const tool of truncatedTools) {
                            if (tool.type === "tool_use" && tool.id) {
                                this.pushToolResultToUserContent({
                                    type: "tool_result",
                                    tool_use_id: tool.id,
                                    content: "This tool was not executed because new_task was called in the same message turn. The new_task tool must be the last tool in a message.",
                                    is_error: true,
                                });
                            }
                        }
    ...
    async getSystemPrompt() {
        ...
        const state = await this.providerRef.deref()?.getState();
        const { mode, customModes, customModePrompts, customInstructions, experiments, language, apiConfiguration, enableSubfolderRules, } = state ?? {};
        return await (async () => {
            const provider = this.providerRef.deref();
            if (!provider) {
                throw new Error("Provider not available");
            ...
            const modelInfo = this.api.getModel().info;
            return SYSTEM_PROMPT(provider.context, this.cwd, false, mcpHub, this.diffStrategy, mode ?? defaultModeSlug, customModePrompts, customModes, customInstructions, experiments, language, rooIgnoreInstructions, {
    ...
    async *attemptApiRequest(retryAttempt = 0, options = {}) {
        ...
        if (contextTokens) {
            ...
            const contextMgmtFilesReadByRoo = contextManagementWillRun && autoCondenseContext
