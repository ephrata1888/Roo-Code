...
describe("editTool", () => {
    ...
    async function executeEditTool(params = {}, options = {}) {
        ...
        await editTool.handle(mockTask, toolUse, {
    ...
    describe("basic replacement", () => {
        it("replaces a single unique occurrence of old_string with new_string", async () => {
            await executeEditTool({ old_string: "Line 2", new_string: "Modified Line 2" }, { fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("replace_all", () => {
        it("replaces all occurrences when replace_all is true", async () => {
            await executeEditTool({ old_string: "Line", new_string: "Row", replace_all: "true" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(mockTask.consecutiveMistakeCount).toBe(0);
            expect(mockTask.diffViewProvider.editType).toBe("modify");
            expect(mockAskApproval).toHaveBeenCalled();
        });
    ...
    describe("uniqueness check", () => {
        it("returns error when old_string appears multiple times without replace_all", async () => {
            const result = await executeEditTool({ old_string: "Line", new_string: "Row" }, { fileContent: "Line 1\nLine 2\nLine 3" });
    ...
    describe("no match error", () => {
        it("returns error when old_string is not found in the file", async () => {
            const result = await executeEditTool({ old_string: "NonExistent", new_string: "New" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
    ...
    describe("old_string equals new_string", () => {
        it("returns error when old_string and new_string are identical", async () => {
            const result = await executeEditTool({ old_string: "Line 2", new_string: "Line 2" }, { fileContent: "Line 1\nLine 2\nLine 3" });
            expect(result).toContain("Error:");
