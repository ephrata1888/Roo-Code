...
vi.mock("../../../utils/storage", () => ({
    getTaskDirectoryPath: vi.fn((globalStoragePath, taskId) => {
...
describe("ReadCommandOutputTool", () => {
    ...
    describe("Basic read functionality", () => {
        it("should read artifact file correctly", async () => {
            ...
            expect(fs.access).toHaveBeenCalledWith(path.join(globalStoragePath, "tasks", taskId, "command-output", artifactId));
        ...
        it("should include size metadata in output", async () => {
            ...
            const fileSize = 5000;
            const buffer = Buffer.from(content);
            vi.mocked(fs.stat).mockResolvedValue({ size: fileSize });
            mockFileHandle.read.mockImplementation((buf) => {
                buffer.copy(buf);
                return Promise.resolve({ bytesRead: buffer.length });
            });
    ...
    describe("Error handling", () => {
        it("should return error for non-existent artifact", async () => {
            ...
            expect(mockCallbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("Error: Artifact not found"));
        });
        it("should reject invalid artifact_id with path traversal attempt", async () => {
            ...
            for (const invalidId of invalidIds) {
                ...
                expect(mockTask.didToolFailInCurrentTurn).toBe(true);
                expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("Invalid artifact_id format"));
            }
        });
        ...
        it("should handle invalid offset gracefully", async () => {
            ...
            const fileSize = 1000;
            vi.mocked(fs.stat).mockResolvedValue({ size: fileSize });
            await tool.execute({ artifact_id: artifactId, offset: 2000 }, // Offset beyond file size
            mockTask, mockCallbacks);
        ...
        it("should handle missing global storage path", async () => {
            ...
            await tool.execute({ artifact_id: artifactId }, mockTask, mockCallbacks);
            expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("Global storage path is not available"));
    ...
    describe("Byte formatting", () => {
        it("should format bytes correctly", async () => {
            ...
            for (const { size, expected } of testCases) {
                ...
                const content = "x";
                const buffer = Buffer.from(content);
                vi.mocked(fs.stat).mockResolvedValue({ size });
                mockFileHandle.read.mockImplementation((buf) => {
                    buffer.copy(buf);
                    return Promise.resolve({ bytesRead: buffer.length });
                });
