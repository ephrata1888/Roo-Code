...
vi.mock("../../prompts/responses", () => ({
    formatResponse: {
        ...
        rooIgnoreError: vi.fn((filePath) => `Access to ${filePath} is blocked by the .rooignore file settings. You must try to continue in the task without using this file, or ask the user to update the .rooignore file.`),
...
describe("ReadFileTool", () => {
    ...
    describe("input validation", () => {
        it("should return error when path is missing", async () => {
            ...
            await readFileTool.execute({ path: "" }, mockTask, callbacks);
        ...
        it("should return error when path is undefined", async () => {
            const mockTask = createMockTask();
            const callbacks = createMockCallbacks();
            await readFileTool.execute({}, mockTask, callbacks);
            expect(mockTask.consecutiveMistakeCount).toBe(1);
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("Error:"));
        });
        it("should return error when offset is 0 or negative", async () => {
            const mockTask = createMockTask();
            const callbacks = createMockCallbacks();
            await readFileTool.execute({ path: "test.txt", offset: 0 }, mockTask, callbacks);
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("offset must be a 1-indexed line number"));
        });
        it("should return error when offset is negative", async () => {
            const mockTask = createMockTask();
            const callbacks = createMockCallbacks();
            await readFileTool.execute({ path: "test.txt", offset: -5 }, mockTask, callbacks);
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("offset must be a 1-indexed line number"));
        });
        it("should return error when anchor_line is 0 or negative", async () => {
            ...
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("anchor_line must be a 1-indexed line number"));
        });
    ...
    describe("RooIgnore handling", () => {
        it("should block access to rooignore-protected files", async () => {
            ...
            const callbacks = createMockCallbacks();
            await readFileTool.execute({ path: "secret.env" }, mockTask, callbacks);
    ...
    describe("directory handling", () => {
        it("should return error when trying to read a directory", async () => {
            ...
            await readFileTool.execute({ path: "src/utils" }, mockTask, callbacks);
            expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("Cannot read 'src/utils' because it is a directory"));
    ...
    describe("image handling", () => {
        ...
        it("should process image file when model supports images", async () => {
            ...
            await readFileTool.execute({ path: "image.png" }, mockTask, callbacks);
        ...
        it("should skip image when model does not support images", async () => {
            ...
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("Model does not support image processing"));
        });
        it("should skip image when file exceeds size limit", async () => {
            ...
            await readFileTool.execute({ path: "large-image.png" }, mockTask, callbacks);
            expect(mockedProcessImageFile).not.toHaveBeenCalled();
            expect(callbacks.pushToolResult).toHaveBeenCalledWith(expect.stringContaining("exceeds the maximum allowed"));
        });
        it("should skip image when total memory limit exceeded", async () => {
            ...
            await readFileTool.execute({ path: "another-image.png" }, mockTask, callbacks);
            expect(mockedProcessImageFile).not.toHaveBeenCalled();
        ...
        it("should handle image read errors gracefully", async () => {
            ...
            await readFileTool.execute({ path: "corrupt.png" }, mockTask, callbacks);
    ...
    describe("binary file handling", () => {
        ...
        it("should extract text from PDF files", async () => {
            ...
            await readFileTool.execute({ path: "document.pdf" }, mockTask, callbacks);
        ...
        it("should extract text from DOCX files", async () => {
            ...
            await readFileTool.execute({ path: "document.docx" }, mockTask, callbacks);
            expect(mockedExtractTextFromFile).toHaveBeenCalled();
        ...
        it("should handle unsupported binary formats", async () => {
            ...
            await readFileTool.execute({ path: "program.exe" }, mockTask, callbacks);
        ...
        it("should handle extraction errors gracefully", async () => {
            ...
            await readFileTool.execute({ path: "corrupt.pdf" }, mockTask, callbacks);
    ...
    describe("text file processing", () => {
        ...
        it("should read text file with slice mode (default)", async () => {
            ...
            await readFileTool.execute({ path: "test.ts" }, mockTask, callbacks);
        ...
        it("should read text file with offset and limit", async () => {
            ...
            await readFileTool.execute({ path: "test.ts", mode: "slice", offset: 2, limit: 2 }, mockTask, callbacks);
        ...
        it("should read text file with indentation mode", async () => {
            ...
            expect(mockedReadWithIndentation).toHaveBeenCalledWith(content, expect.objectContaining({
                anchorLine: 3,
            }));
        });
        it("should show truncation notice when content is truncated", async () => {
            ...
            await readFileTool.execute({ path: "large.ts" }, mockTask, callbacks);
        ...
        it("should handle empty files", async () => {
            ...
            await readFileTool.execute({ path: "empty.ts" }, mockTask, callbacks);
    ...
    describe("approval flow", () => {
        ...
        it("should include user feedback when provided with denial", async () => {
            ...
            await readFileTool.execute({ path: "secrets.env" }, mockTask, callbacks);
    ...
    describe("output structure", () => {
        it("should include file path in output", async () => {
            ...
            await readFileTool.execute({ path: "src/app.ts" }, mockTask, callbacks);
    ...
    describe("error handling", () => {
        it("should handle file read errors gracefully", async () => {
            ...
            await readFileTool.execute({ path: "nonexistent.ts" }, mockTask, callbacks);
        ...
        it("should handle stat errors gracefully", async () => {
            ...
            await readFileTool.execute({ path: "protected.ts" }, mockTask, callbacks);
            expect(mockTask.say).toHaveBeenCalledWith("error", expect.stringContaining("Error reading file"));
            expect(mockTask.didToolFailInCurrentTurn).toBe(true);
        });
    });
