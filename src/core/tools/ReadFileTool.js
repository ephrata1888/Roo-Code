...
export class ReadFileTool extends BaseTool {
    ...
    async executeNew(params, task, callbacks) {
        ...
        try {
            ...
            const { maxImageFileSize = DEFAULT_MAX_IMAGE_FILE_SIZE_MB, maxTotalImageSize = DEFAULT_MAX_TOTAL_IMAGE_SIZE_MB, } = state ?? {};
            for (const fileResult of fileResults) {
                ...
                try {
                    ...
                    if (isBinary) {
                        await this.handleBinaryFile(task, relPath, fullPath, supportsImages, maxImageFileSize, maxTotalImageSize, imageMemoryTracker, updateFileResult);
                        continue;
                    }
     ...
     */
    async handleBinaryFile(task, relPath, fullPath, supportsImages, maxImageFileSize, maxTotalImageSize, imageMemoryTracker, updateFileResult) {
            ...
            try {
                ...
                if (isBinary) {
                    ...
                    if (supportsImages && isSupportedImageFormat(fileExtension)) {
                        const state = await task.providerRef.deref()?.getState();
                        const { maxImageFileSize = DEFAULT_MAX_IMAGE_FILE_SIZE_MB, maxTotalImageSize = DEFAULT_MAX_TOTAL_IMAGE_SIZE_MB, } = state ?? {};
                        const validation = await validateImageForProcessing(fullPath, supportsImages, maxImageFileSize, maxTotalImageSize, 0);
