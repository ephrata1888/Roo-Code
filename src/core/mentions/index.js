...
export async function parseMentions(text, cwd, fileContextTracker, rooIgnoreController, showRooIgnoredFiles = false, includeDiagnosticMessages = true, maxDiagnosticMessages = 50) {
    ...
    for (const mention of mentions) {
        if (mention.startsWith("/")) {
            const mentionPath = mention.slice(1);
            try {
                const fileResult = await getFileOrFolderContentWithMetadata(mentionPath, cwd, rooIgnoreController, showRooIgnoredFiles, fileContextTracker);
 ...
 */
async function getFileOrFolderContentWithMetadata(mentionPath, cwd, rooIgnoreController, showRooIgnoredFiles = false, fileContextTracker) {
