 ...
 */
export function toolUseToText(block) {
    let input;
 ...
 * This allows the conversation to be summarized without requiring the tools parameter.
 */
export function toolResultToText(block) {
 ...
 */
export function convertToolBlocksToText(content) {
 ...
 */
export function transformMessagesForCondensing(messages) {
...
export async function summarizeConversation(options) {
    const { messages, apiHandler, systemPrompt, taskId, isAutomaticTrigger, customCondensingPrompt, metadata, environmentDetails, filesReadByRoo, cwd, rooIgnoreController, } = options;
    ...
    const messagesWithTextToolBlocks = transformMessagesForCondensing(maybeRemoveImageBlocks([...messagesWithToolResults, finalRequestMessage], apiHandler));
