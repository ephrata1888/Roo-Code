...
1--3 | export async function fetchData(url: string): Promise<any>
            ...
            const userFileBlock = contentArray.find((block) => block.type === "text" && block.text?.includes("## File Context: src/user.ts"));
            const apiFileBlock = contentArray.find((block) => block.type === "text" && block.text?.includes("## File Context: src/api.ts"));
        ...
        it("should not include file context section when filesReadByRoo is empty", async () => {
            ...
            const contentArray = summaryMessage.content;
            const fileContextBlock = contentArray.find((block) => block.type === "text" && block.text?.includes("## File Context"));
