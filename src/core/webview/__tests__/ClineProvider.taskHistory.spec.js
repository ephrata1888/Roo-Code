...
describe("ClineProvider Task History Synchronization", () => {
    ...
    describe("updateTaskHistory", () => {
        ...
        it("invalidates recentTasksCache on updateTaskHistory (UTH-04)", async () => {
            ...
            await provider.updateTaskHistory(createHistoryItem({
                id: "cache-seed",
                task: "Cache seed",
                workspace,
                ts: tsBase,
    ...
    describe("taskHistory write lock (mutex)", () => {
        it("serializes concurrent updateTaskHistory calls so no entries are lost", async () => {
            ...
            const items = Array.from({ length: 5 }, (_, i) => createHistoryItem({ id: `concurrent-${i}`, task: `Task ${i}` }));
            await Promise.all(items.map((item) => provider.updateTaskHistory(item, { broadcast: false })));
            // All 5 entries must survive
            const history = provider.contextProxy.getGlobalState("taskHistory");
        ...
        it("does not block subsequent writes when a previous write errors", async () => {
            ...
            const origUpdateGlobalState = provider.updateGlobalState.bind(provider);
            let callCount = 0;
            provider.updateGlobalState = vi.fn().mockImplementation((...args) => {
                callCount++;
                if (callCount === 1) {
            ...
            await expect(provider.updateTaskHistory(item1, { broadcast: false })).rejects.toThrow("simulated write failure");
