...
import { RooCodeEventName, requestyDefaultModelId, openRouterDefaultModelId, DEFAULT_WRITE_DELAY_MS, ORGANIZATION_ALLOW_ALL, DEFAULT_MODES, DEFAULT_CHECKPOINT_TIMEOUT_SECONDS, getModelId, isRetiredProvider, } from "@roo-code/types";
...
export class ClineProvider extends EventEmitter {
    ...
    _disposed = false;
    recentTasksCache;
    taskHistoryWriteLock = Promise.resolve();
    ...
    settingsImportedAt;
    latestAnnouncementId = "jan-2026-v3.45.0-smart-code-folding"; // v3.45.0 Smart Code Folding
            ...
            if (parentTaskId && childTaskId && !options?.skipDelegationRepair) {
                try {
                    ...
                    if (parentHistory.status === "delegated" && parentHistory.awaitingChildId === childTaskId) {
                        ...
                        this.log(`[ClineProvider#removeClineFromStack] Repaired parent ${parentTaskId} metadata: delegated → active (child ${childTaskId} removed)`);
                    }
                }
                catch (err) {
                    // Non-fatal: log but do not block the pop operation.
                    this.log(`[ClineProvider#removeClineFromStack] Failed to repair parent metadata for ${parentTaskId} (non-fatal): ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        }
    }
    ...
    */
    ...
    async resolveWebviewView(webviewView) {
        ...
        // global state values.
        this.getState().then(({ terminalShellIntegrationTimeout = Terminal.defaultShellIntegrationTimeout, terminalShellIntegrationDisabled = false, terminalCommandDelay = 0, terminalZshClearEolMark = true, terminalZshOhMy = false, terminalZshP10k = false, terminalPowershellCounter = false, terminalZdotdir = false, ttsEnabled, ttsSpeed, }) => {
    ...
    async getTaskWithId(id) {
        ...
        if (fileExists) {
            try {
                ...
                console.warn(`[getTaskWithId] api_conversation_history.json corrupted for task ${id}, returning empty history: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        else {
        ...
        try {
            ...
            for (const taskId of allIdsToDelete) {
                try {
                    ...
                    console.error(`[deleteTaskWithId${taskId}] failed to delete associated shadow repository or branch: ${error instanceof Error ? error.message : String(error)}`);
                }
                ...
                try {
                    ...
                    console.error(`[deleteTaskWithId${taskId}] failed to remove task directory: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            await this.postStateToWebview();
        }
    ...
    async getState() {
        ...
        const apiProvider = stateValues.apiProvider && !isRetiredProvider(stateValues.apiProvider)
     ...
     */
    withTaskHistoryLock(fn) {
        const result = this.taskHistoryWriteLock.then(fn, fn); // run even if previous write errored
        this.taskHistoryWriteLock = result.then(() => { }, () => { }); // swallow for chain continuity
        return result;
    }
    /**
    ...
    async delegateParentAndOpenChild(params) {
        ...
        try {
            ...
            if (!flushSuccess) {
                ...
                if (!retrySuccess) {
                    console.error(`[delegateParentAndOpenChild] CRITICAL: Parent ${parentTaskId} API history not persisted to disk. Child return may produce stale state.`);
                    vscode.window.showWarningMessage("Warning: Parent task state could not be saved. The parent task may lose recent context when resumed.");
                }
            }
        }
        catch (error) {
