...
import { providerSettingsWithIdSchema, discriminatedProviderSettingsWithIdSchema, isSecretStateKey, DEFAULT_CONSECUTIVE_MISTAKE_LIMIT, getModelId, isProviderName, isRetiredProvider, } from "@roo-code/types";
...
export class ProviderSettingsManager {
    ...
    async saveConfig(name, config) {
        try {
            return await this.lock(async () => {
                ...
                const filteredConfig = typeof config.apiProvider === "string" && isRetiredProvider(config.apiProvider)
    ...
    async load() {
        try {
            ...
            const apiConfigs = Object.entries(providerProfiles.apiConfigs).reduce((acc, [key, apiConfig]) => {
                ...
                const providerValue = typeof sanitizedConfig === "object" &&
                    sanitizedConfig !== null &&
                    "apiProvider" in sanitizedConfig
                    ? sanitizedConfig.apiProvider
                    : undefined;
                const schema = typeof providerValue === "string" && isRetiredProvider(providerValue)
    ...
    sanitizeProviderConfig(apiConfig) {
        ...
        if (apiProvider !== undefined &&
            ...
            console.log(`[ProviderSettingsManager] Sanitizing unknown provider "${config.apiProvider}" - resetting to undefined`);
