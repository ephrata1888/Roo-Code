...
const { mockStat, mockReadFile, mockReaddir, mockHomedir, mockDirectoryExists, mockFileExists, mockRealpath, mockMkdir, mockWriteFile, mockRm, mockRename, mockRmdir, } = vi.hoisted(() => ({
    mockStat: vi.fn(),
    mockReadFile: vi.fn(),
    mockReaddir: vi.fn(),
    mockHomedir: vi.fn(),
...
vi.mock("../../../i18n", () => ({
    t: (key, params) => {
        const translations = {
            ...
            "skills:errors.name_format": "Skill name must be lowercase letters/numbers/hyphens only (no leading/trailing hyphen, no consecutive hyphens)",
...
Instructions`);
    ...
    describe("createSkill", () => {
        ...
        it("should throw error for invalid skill name", async () => {
            await expect(skillsManager.createSkill("Invalid-Name", "global", "Description")).rejects.toThrow("Skill name must be lowercase letters/numbers/hyphens only");
        });
        it("should throw error for skill name that is too long", async () => {
            const longName = "a".repeat(65);
            await expect(skillsManager.createSkill(longName, "global", "Description")).rejects.toThrow("Skill name must be 1-64 characters");
        });
        it("should throw error for skill name starting with hyphen", async () => {
            await expect(skillsManager.createSkill("-invalid", "global", "Description")).rejects.toThrow("Skill name must be lowercase letters/numbers/hyphens only");
        });
        ...
        it("should throw error for empty description", async () => {
            await expect(skillsManager.createSkill("valid-name", "global", "   ")).rejects.toThrow("Skill description must be 1-1024 characters");
        });
        it("should throw error for description that is too long", async () => {
            const longDesc = "d".repeat(1025);
            await expect(skillsManager.createSkill("valid-name", "global", longDesc)).rejects.toThrow("Skill description must be 1-1024 characters");
        });
        it("should throw error if skill already exists", async () => {
            mockFileExists.mockResolvedValue(true);
            await expect(skillsManager.createSkill("existing-skill", "global", "Description")).rejects.toThrow("already exists");
        });
    });
...
Instructions`);
        ...
        it("should throw error if skill does not exist", async () => {
            ...
            mockReaddir.mockResolvedValue([]);
            await skillsManager.discoverSkills();
            await expect(skillsManager.moveSkill("non-existent", "global", undefined, "code")).rejects.toThrow("not found");
        });
...
Instructions`);
            await skillsManager.discoverSkills();
            await expect(skillsManager.moveSkill("test-skill", "global", undefined, "code")).rejects.toThrow("already exists");
        });
...
Instructions`);
        ...
        it("should not clean up source skills directory if it still has other skills", async () => {
            ...
            mockFileExists.mockImplementation(async (file) => {
                ...
                if (file === testSkillMd)
                    return true;
                if (file === p(sourceSkillsDir, "another-skill", "SKILL.md"))
                    return true;
                // Skill does not exist in destination
                if (file === p(destDir, "SKILL.md"))
                    return false;
