 ...
 * 6. Apply line limit
 */
import { DEFAULT_LINE_LIMIT, DEFAULT_MAX_LEVELS, MAX_LINE_LENGTH, } from "../../core/prompts/tools/native-tools/read_file";
// ─── Constants ────────────────────────────────────────────────────────────────
 ...
 */
export function computeEffectiveIndents(lines) {
    const effective = [];
 ...
 */
export function formatWithLineNumbers(lines, maxLineLength = MAX_LINE_LENGTH) {
    if (lines.length === 0)
        return "";
 ...
 */
export function readWithIndentation(content, options) {
    const { anchorLine, maxLevels = DEFAULT_MAX_LEVELS, includeSiblings = false, includeHeader = true, limit = DEFAULT_LINE_LIMIT, maxLines, } = options;
 ...
 * @returns The extracted content with metadata
 */
export function readWithSlice(content, offset = 0, limit = DEFAULT_LINE_LIMIT) {
    const lines = parseLines(content);
    const totalLines = lines.length;
    // Validate offset
    if (offset < 0)
