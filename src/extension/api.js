...
export class API extends EventEmitter {
            ...
            ipc.on(IpcMessageType.TaskCommand, async (clientId, command) => {
                ...
                switch (command.commandName) {
                    case TaskCommandName.StartNewTask:
                        this.log(`[API] StartNewTask -> ${command.data.text}, ${JSON.stringify(command.data.configuration)}`);
    ...
    async resumeTask(taskId) {
        ...
        if (this.sidebarProvider.viewLaunched) {
            ...
            this.log(`[API#resumeTask] webview not launched after resume for task ${taskId}; continuing in headless mode`);
        }
    }
